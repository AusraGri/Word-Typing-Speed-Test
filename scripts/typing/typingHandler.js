export const typingTest = {
  ...
  highlightCurrentLetter: function () {
    ...
    const nextletterDivs = this.getLetterDivs(this.currentWordIndex + 1)
    nextletterDivs.forEach((div) => {
      div.classList.remove("highlight")
    })
  },
  ...
  moveToPreviousLetter: function () {
    ...
    if (currentLetterDiv.classList.contains("correct")) {
      this.correctLettersCount--
    } else if (currentLetterDiv.classList.contains("incorrect")) {
      this.incorrectLettersCount--
    }
    this.totalLettersCount--
